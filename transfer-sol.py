...
if len(sys.argv) != 3 or '-?' in sys.argv:
    if len(sys.argv) != 1:
        ...
        print('\n      AMOUNT,RECIPIENT_WALLET_ADDRESS\n')
...
# open the log file
log_file = open(datetime.now().strftime('transfer-sol %d-%m-%Y %H-%M-%S.log'), 'w')
logging.info('opened log file at ' + datetime.now().strftime('transfer-sol %d-%m-%Y %H-%M-%S.log'))

# loop over all of the recipients
for record in records:
    # initialize the variables and prepare to send
    if len(record) == 0 or record[0] == '' or record[1] == '':
        logging.info('encountered blank entry in CSV file, continuing to next record')
        continue
    amount, recipient = record[0], record[1]
    logging.info('sending ' + amount + ' of SOL to recipient ' + recipient)
    ...
    try:        
        output = get_proc_output('solana transfer --allow-unfunded-recipient ' + recipient + ' ' + amount) 
